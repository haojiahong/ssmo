...
<c:forEach items="${articleList}" var="article">
	<article class="excerpt excerpt-one">
		...
		<p class="focus">
			<a target="_blank" href="<%=basePath%>article/html/${article.id}"
				class="thumbnail"> <span class="item"> <span
					class="thumb-span"> <c:if test="${article.imageUrl !=null}">
							<!--<img data-original="<%=basePath%>uploads/${article.imageUrl}" class="thumb" src="<%=basePath%>uploads/loading.jpg" style="display: inline;">-->
							<img
								data-original="http://7xljkx.com1.z0.glb.clouddn.com/uploads/${article.imageUrl}"
								class="thumb"
								src="http://7xljkx.com1.z0.glb.clouddn.com/uploads/loading.jpg"
		...
		<p class="text-muted views">
			<span class="post-views">阅读(${article.showCount})</span> <span
				class="post-tags">标签： <c:forEach items="${article.tagList}"
					...
					<a href="javascript:void(0)" onclick="goTag('${tag.tagName}')"
						rel="tag" data-original-title="" title="">${tag.tagName}</a>
