package com.hjh.ssmo.util.thread;
...
import com.hjh.ssmo.util.HttpTookit;
import com.hjh.ssmo.util.spring.SpringContext;

/**
 * @desc 栏目首页缓存线程 每隔一段时间自动刷新一次 主要是为了 如果缓存过期 后台自动刷新 不需要用户去点击生成缓存 生成缓存的过程体验差
 * @author ljt
...
public class CategoryThread implements Runnable {
	...
	private void updateCategoryCache() {
		...
		for (Category category : categoryList) {
			// 调用自身接口
			String categoryPageUrl = HOST_URL + "/article/loadPage/" + category.getId().intValue() + "/99999999999";
			HttpTookit.doGet(categoryPageUrl, null);

			if (category.getAliasName() != null) {
				String categoryUrl = HOST_URL + "/category/" + category.getAliasName();
